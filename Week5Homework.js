...
const phoneNumbers = userData.map(userData => {
  return userData.phone;
});
console.log(phoneNumbers);
//create and print a list of website and email pairs (pair them in a string, array, or object)
const webEmail = userData.map(userData => [userData.website, userData.email]);
...
const toDo = toDoData.filter(toDoData => toDoData.userId == 9);
console.log(toDo);
//then find the number of user 9's incomplete tasks
const incompleteTask = toDo.filter(toDo => toDo.completed == false);
...
const fivePostData = postData.filter(postData => postData.userId == 5);
console.log(fivePostData);
//then, create a list of title and body pairs
const titleBody = postData.map(postData => [postData.title, postData.body]);
